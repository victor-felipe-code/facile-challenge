...
const serviceCreate = async name => {
  const resultValidate = (0, _helpers.validateString)(name);

  if (resultValidate !== null && resultValidate !== void 0 && resultValidate.error) {
    return {
      statusCode: _helpers.HttpStatusCode.UNPROCESSABLE_ENTITY,
      message: resultValidate.error
    };
  }

  const resultModel = await (0, _model.modelCreateName)((0, _helpers.encrypt)(name));
  return {
    statusCode: 201,
    created: {
      id: resultModel.id,
...
const serviceGetById = async id => {
  const resultValidate = (0, _helpers.validateId)(id);
  ...
  if (!(resultModel !== null && resultModel !== void 0 && resultModel.name) || !(resultModel !== null && resultModel !== void 0 && resultModel.id)) {
    return {
      statusCode: _helpers.HttpStatusCode.NOT_FOUND,
      message: {
        code: "E_VALIDATION_FAILURE",
        message: _helpers.MESSAGE_CUSTOM.ID_NOT_EXISTS
      }
    };
  }

  const textDecrypted = (0, _helpers.decrypt)(resultModel.name);
  return {
    statusCode: 200,
    content: {
